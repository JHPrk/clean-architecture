...
public class Restaurant extends AggregateRoot<RestaurantId> {
  ...
  public void validateOrder(List<String> failureMessages) {
    if (orderDetail.getOrderStatus() != OrderStatus.PAID) {
      failureMessages.add(
          String.format("Payment is not completed for order: {0}", orderDetail.getId()));
    }
    Money totalAmount = orderDetail.getProducts().stream().map(product -> {
      if (product.isAvailable()) {
        failureMessages.add(
            String.format("Product with id: {0} is not available", product.getId().getValue()));
      }
    ...
    if (!totalAmount.equals(orderDetail.getTotalAmount())) {
      failureMessages.add(
          String.format("Price total is not correct for order: {0}", orderDetail.getId()));
    }
  }
